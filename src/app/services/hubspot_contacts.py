...
class AccessTokenProvider(Protocol):
    async def get_access_token(self, tenant_id: str) -> tuple[str, StoredOAuthToken]: ...
...
class HubSpotAccessTokenProvider:
    ...
    async def get_access_token(self, tenant_id: str) -> tuple[str, StoredOAuthToken]:
        ...
        if existing.expires_at > datetime.now(UTC) + timedelta(minutes=1):
            return self._cipher.decrypt(existing.encrypted_access_token), existing
    ...
    async def _get_token(self, tenant_id: str) -> StoredOAuthToken | None:
        async with UnitOfWork(self._session_factory) as unit_of_work:
            repository = self._repository(unit_of_work)
            return await repository.get_token(tenant_id)

    def _repository(self, unit_of_work: UnitOfWork) -> OAuthTokenRepository:
